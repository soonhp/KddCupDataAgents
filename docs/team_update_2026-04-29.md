...
### 2.9 Retry / Retry Orchestration 흐름
`runner/retry_orchestrator.py`, `runner/attempt_selector.py`

repair execution 이후에도 남은 문제는 다음 solver pass용 retry brief로 정리하고, 이후 시도 간 비교를 위한 준비를 한다.

현재 개념적으로 다루는 focus 예시
...
- retry 여부를 구조화된 decision으로 남김
- retry용 instruction / note / plan artifact를 준비함
- original attempt와 post-repair/retry attempt를 비교할 수 있는 selection 흐름을 둠
...
11. retry decision / retry artifacts 준비
12. attempt selection 정보 계산
13. 모든 결과를 `task.log.json`에 기록
...
- retry 관련 stale instruction / outdated state 반영 문제를 리뷰 피드백 기준으로 점검
...
작업 중 connector 상에서 특정 PR head commit에 대해 GitHub Actions status / workflow run이 바로 보이지 않는 경우가 있었다.
...
- `runner/repair_planner.py`
- `runner/repair_executor.py`
- `runner/retry_orchestrator.py`
- `runner/attempt_selector.py`
...
- `tests/test_attempt_selector.py`
- `tests/test_three_way_attempt_selection.py`
...
1. retry decision / retry artifacts가 준비되더라도 **실제 second-pass solver invocation 자동화**는 더 정교화가 필요함
2. safe deterministic repair는 여전히 보수적이며, 값 생성은 하지 않음
...
### A. Repair-aware retry orchestration 고도화
- retry instruction을 실제 second-pass solver context에 더 정교하게 주입
- retry input artifact와 selection artifact를 더 명시적으로 연결
- failure taxonomy와 retry reason의 매핑 강화

### B. Attempt selection 고도화
- retry / post-repair / original attempt 간 비교 기준을 더 세분화
- semantic quality와 contract quality의 가중치 기준 명확화
- 최종 선택 근거를 사람에게 더 읽기 쉬운 형태로 기록
...
8. `runner/retry_orchestrator.py`
9. `runner/attempt_selector.py`

---

## 10. 한 줄 요약

이제 KDD Data Agents runner는 **실행 → 검증 → 의미 점검 → agent 리뷰 → repair 계획 → safe repair → retry orchestration 준비 → attempt selection**까지 이어지는, 비교적 운영 가능한 deterministic evaluation pipeline으로 확장되었다.
